...
% se é estado terminal  não há jogada
%minimax_decidir(_,_,terminou).         
minimax_decidir(Ei,terminou):- terminal(Ei, Jog),!, write('Vencedor: '),write(Jog),nl,nl.
minimax_decidir(Ei,terminou):- empate(Ei),!, write('Empate!'),nl,nl.

%Para cada estado sucessor de Ei calcula o valor minimax do estado
%Opf é o operador (jogada) que tem maior valor

%minimax_decidir(Ultimo jogador a ter jogado,_,_).

minimax_decidir(o,Ei,Opf):-
...
minimax_decidir(x,Ei,Opf):-
          findall(Es-Op, op1(Ei,Op,Es),L),
          findall(Vc-Op,(member(E-Op,L), minimax_valor(E,Vc,2)),L1),
          escolhe_min(L1,Opf).

% se um estado é terminal o valor é dado pela função de utilidade
minimax_valor(Ei,Val,P):- terminal(Ei, Jog), valor(Ei,Val,P).
...
escolhe_min([A|R],Val):- escolhe_min(R,A,Val).

escolhe_min([],_-Op,Op).
escolhe_min([A-_|R],X-Op,Val):- A > X,!, escolhe_min(R,X-Op,Val).
escolhe_min([A|R],_,Val):-  escolhe_min(R,A,Val).
